...
uint8_t aFileName[FILE_NAME_LENGTH];//文件
...
HAL_StatusTypeDef Serial_PutByte( uint8_t param )
{
  ...
  return HAL_UART_Transmit(&huart1, &param, 1, HAL_MAX_DELAY);
}
...
/**
  ...
  * @param  data
  * @param  length
  *     0: end of transmission序号和补码校验不成功
  *     2: abort by sender发送方中止传输
  *    >0: packet length正常的数据包长度
  * @param  timeout
...
static HAL_StatusTypeDef ReceivePacket(uint8_t *p_data, uint32_t *p_length, uint32_t timeout)
{
  ...
  HAL_StatusTypeDef status;//函数运行状态返回值
  uint8_t char1;//接收缓冲区
  *p_length = 0;
  status = HAL_UART_Receive(&huart1, &char1, 1, timeout);//阻塞接收

  if (status == HAL_OK)//如果成功接收
  {
    switch (char1)
    {
      case SOH://128b包大小的数据包头 起始帧
        packet_size = PACKET_SIZE;//标记包大小
        break;
      case STX://1K包大小的数据包头 数据帧
